...
/*********************************************************************
* getText(node)
* Returns a string containing all the text in the node.
* Returns an empty string if the node does not have any text.
*
* Params:
* Name	Type	Description
* node 	Object	The node in which to search for text.
*
* Example:
* var txt = "";
* txt = getText(document.p[0]);
*********************************************************************/
function getText(node) {
	var txt = "";
	return getTextSub(node, txt);
}

//recursive helper function
function getTextSub(node, txt) {
	var children = node.childNodes;
	var index;
	for (index = 0; index < children.length; index++) {
		var thisNode = children[index];
		
		if (thisNode.nodeType == 3) {
			txt = txt.concat(thisNode.nodeValue);
		}
		else {
			txt = getTextSub(thisNode, txt);
		}
	}
	return txt;
}
